...
module.exports = {
  ...
  after: {
    ...
    create: [ when(hook => hook.result && hook.result.scope === 'members' && hook.result.count === 1, hooks.createTopic) ],
    update: [],
    patch: [],
    // We remove topics for members tag only, check if tag is really removed or just its count decreased
    remove: [ when(hook => hook.result && hook.result.scope === 'members' && hook.result.count <= 0, hooks.removeTopic) ]
  },
