...
then
	...
	scp deploy/scripts/app.deploy.sh ${SSH_USER}@${SSH_REMOTE}:~/${APP}/deploy.sh
	scp deploy/scripts/app.remove.sh ${SSH_USER}@${SSH_REMOTE}:~/${APP}/remove.sh
	scp deploy/scripts/app.backup.sh ${SSH_USER}@${SSH_REMOTE}:~/${APP}/backup.sh
	scp deploy/scripts/app.restore.sh ${SSH_USER}@${SSH_REMOTE}:~/${APP}/restore.sh
	scp deploy/scripts/app.backup.yml ${SSH_USER}@${SSH_REMOTE}:~/${APP}/backup.yml
